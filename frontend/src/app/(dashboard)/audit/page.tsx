...
import { ScrollText, RefreshCw, Loader2, ShieldAlert, ShieldCheck, User, Search } from "lucide-react";
...
const ROLES = ["ALL", "OWNER", "MANAGER", "TRAINER", "MEMBER"];
...
export default function AuditPage() {
  ...
  useEffect(() => {
    const t = setTimeout(() => setSearch((s) => s), 0);
  ...
  }, [module, role, result, search, limit, refreshKey]);
  ...
  return (
    <div className="space-y-5">
      <AuditLogDrawer log={activeLog} onClose={() => setActiveLog(null)} />
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">System activity, security events, and actor trail</p>
          </div>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-xl border border-border bg-white p-4">
          ...
          <p className="mt-3 text-2xl font-bold text-slate-900">{loading ? "..." : kpis.todayCount}</p>
        ...
        <div className="rounded-xl border border-border bg-white p-4">
          ...
          <p className="mt-3 text-2xl font-bold text-slate-900">{loading ? "..." : kpis.failures}</p>
        ...
        <div className="rounded-xl border border-border bg-white p-4">
          ...
          <p className="mt-3 text-2xl font-bold text-slate-900">{loading ? "..." : kpis.actors}</p>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative w-full max-w-xs">
          ...
          <input
            ...
            className="h-9 w-full rounded-xl border border-border bg-white pl-9 pr-3 text-xs focus:border-violet-500 focus:outline-none"
          />
        </div>
        <select value={module} onChange={(e) => setModule(e.target.value)} className="h-9 rounded-xl border border-border bg-white px-2 text-xs">
          {MODULES.map((m) => (
        ...
        <select value={role} onChange={(e) => setRole(e.target.value)} className="h-9 rounded-xl border border-border bg-white px-2 text-xs">
          {ROLES.map((r) => (
        ...
        <select value={result} onChange={(e) => setResult(e.target.value)} className="h-9 rounded-xl border border-border bg-white px-2 text-xs">
          {RESULTS.map((r) => (
            <option key={r} value={r}>
              {r === "ALL" ? "All results" : r}
            </option>
        ...
        <select value={limit} onChange={(e) => setLimit(Number(e.target.value))} className="h-9 rounded-xl border border-border bg-white px-2 text-xs">
          {[50, 100, 150].map((n) => (
        ...
        {loading ? (
          ...
          <table className="min-w-full divide-y divide-border text-left text-xs">
            ...
            <tbody className="divide-y divide-border">
              {logs.map((log) => (
                <tr key={log.id} className="cursor-pointer hover:bg-slate-50" onClick={() => setActiveLog(log)}>
                  ...
                  <td className="whitespace-nowrap px-4 py-2.5 text-slate-600">{log.role ?? "—"}</td>
                  <td className="whitespace-nowrap px-4 py-2.5 text-slate-600">{log.module ?? "—"}</td>
                  <td className="whitespace-nowrap px-4 py-2.5 font-medium text-slate-700">{log.action ?? "—"}</td>
                  <td className="max-w-[220px] truncate px-4 py-2.5 text-slate-500" title={log.details ?? undefined}>
                    {log.details ?? "—"}
                  </td>
      ...
      {!loading && (
        <p className="text-[11px] text-slate-400">
          Showing {logs.length} of {count} events · select a row to open the detail drawer
        </p>
      )}
    </div>
  );
